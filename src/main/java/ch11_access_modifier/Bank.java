...
/*
    1. Bank 클래스
        ...
        3) 메서드
            ...
            BankMain 클래스에 객체를 만들어서 setter /  getter 를 사용하여 계좌 정보 및 출력을 해보고,
            deposit() / withdraw()를 실행 시켜 입금 성공 및 실패 사례를 확인해보고 통과되는지 검증해보세요.
...
public class Bank {
    ...
    public void setPin(String pin) {
        if(pin.length() == 4 && (Integer.parseInt(pin) >= 0 ) && (Integer.parseInt(pin) < 10000) ) {
            this.pin = pin;
        } else {
    ...
    public void setBalance(int balance) {
        if(balance < 0) {
            System.out.println("잔액은 음수가 입력될 수 없습니다.");
        } else {
            this.balance = balance;
        }
    }
